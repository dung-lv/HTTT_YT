 ...
 */
Ext.define('QLDT.view.project.ProjectSurveyController', {
    extend: 'QLDT.view.base.BaseListController',

    requires: ['QLDT.view.base.BaseListController'],

    alias: 'controller.ProjectSurvey',
                    ...
                    store.load({
                        callback: function (records, operation, success) {
                            ...
                            if (Ext.isArray(records) && records.length > 0) {
                                me.showLinkDownloadPlanAttach(records[0].data.ProjectSurveyID, me.projectID, records[0].data.FileType);
                            }
                        }
                    });
                }
                ...
                if (proxy) {
                    ...
                    store.load({
                        callback: function (records, operation, success) {
                            ...
                            Ext.each(Ext.select(".grid-file-servey .file-download").elements, function (element) {
  ...
  */
        ...
        if (form) {
            ...
            form.submitWithFile(function (rep) {
                if (rep && rep.Success && Ext.isArray(rep.Data) && rep.Data.length > 0) {
                    ...
                    var uri = QLDT.utility.Utility.createUriAPI("ProjectSurvey");
    ...
    showLinkDownloadPlanAttach: function (id,projectID, fileType) {
        ...
        if (linkDowload) {
            var uriDownload = Ext.String.format(QLDT.utility.Utility.createUriAPI('/Download?TokenID={0}&ID={1}&FT={2}&IsTemp={3}&DF={4}'),
             ...
             strLink = Ext.String.format('<a class="link-download" target="_blank" href="{0}">Tải tệp kế hoạch khảo sát</a><a id="deleteFileProjectSurvey" class="cls-delete-file fa fa-trash-o" projectsurveyid="{1}" href="javascript:void(0)">Xóa tệp</a>', uriDownload, id);
            linkDowload.setHtml(strLink);
            linkDowload.setHidden(false);
            Ext.get('deleteFileProjectSurvey').on('click', function (e) {
                var projectsurveyid = e.target.getAttribute("projectsurveyid");
                if (projectsurveyid) {
                    var uri = QLDT.utility.Utility.createUriAPI("ProjectSurvey");
                    QLDT.utility.MTAjax.request(uri, "DELETE", {}, [projectsurveyid], function (respone) {
                        if (respone && respone.Success) {
   ...
   */
    getPageDetail: function () {
        return 'QLDT.view.project.ProjectSurveyDetail';
    },
